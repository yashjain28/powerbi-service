function SetupPowerBI(req, resp) {
    ...
    var checkConstants = function () {
        ...
        if (checkConstantEmpty(PLATFORM_URL)) {
            response.err = true;
            response.messages.push("PLATFORM_URL not set in PowerBI Library");
        }
        if (checkConstantEmpty(POWER_BI_URL)) {
            response.err = true;
            response.messages.push("POWER_BI_URL not set in PowerBI Library");
        }
    };
    ...
    var main = function () {
        ...
        if (response.err === true) {
            ...
            createUser("powerbiuser@clearblade.com", portalUserPassword, "PowerBIUser", "");
            sendResponse();
        }
    };
